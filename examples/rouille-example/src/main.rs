...
use oxide_auth::endpoint::{
    AccessTokenFlow, AuthorizationFlow, OwnerConsent, PreGrant, RefreshFlow, ResourceFlow,
};
use oxide_auth::frontends::rouille::{FnSolicitor, GenericEndpoint, OAuthRequest, OAuthResponse};
...
pub fn main() {
    ...
    let endpoint = Mutex::new(GenericEndpoint {
        ...
        response: OAuthResponse::empty_404,
    });
        ...
        router!(request,
                    (GET) ["/"] => {
                        ...
                        { // Does not have the proper authorization token
                            let mut response = err.unwrap_or_else(|_| Response::empty_400().into());
        let text = "<html>
...
";
                            response.0.data = ResponseBody::from_string(text);
                            response.0.with_unique_header("Content-Type", "text/html; charset=utf8")
                    ...
                    (GET) ["/authorize"] => {
                        ...
                        AuthorizationFlow::prepare(&mut *locked)
                            .expect("Can not fail")
                            .execute(OAuthRequest(request))
                            .unwrap_or_else(|_| Response::empty_400().into()).into()
                    },
                    (POST) ["/authorize"] => {
                        let mut locked = endpoint.lock().unwrap();
                        AuthorizationFlow::prepare(&mut *locked)
...
/// the flow.
fn solicitor(request: &mut OAuthRequest, grant: &PreGrant) -> OwnerConsent<OAuthResponse> {
