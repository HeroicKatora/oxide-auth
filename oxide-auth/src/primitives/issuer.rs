...
pub trait Issuer {
    ...
    fn recover_token<'a>(&'a mut self, &'a str) -> Result<Option<Grant>, ()>;

    /// Get the values corresponding to a refresh token
    fn recover_refresh<'a>(&'a mut self, &'a str) -> Result<Option<Grant>, ()>;
}
...
impl IssuedToken {
    ...
    ///     }
    ///     // …
    /// # fn recover_token<'t>(&'t mut self, token: &'t str) -> Result<Option<Grant>, ()> { Err(()) }
    /// # fn recover_refresh<'t>(&'t mut self, token: &'t str) -> Result<Option<Grant>, ()> { Err(()) }
    /// }
    /// ```
...
impl<G: TagGrant> Issuer for TokenMap<G> {
    ...
    fn recover_token<'a>(&'a mut self, token: &'a str) -> Result<Option<Grant>, ()> {
        Ok(self.access.get(token).map(|token| token.grant.clone()))
    }

    fn recover_refresh<'a>(&'a mut self, token: &'a str) -> Result<Option<Grant>, ()> {
