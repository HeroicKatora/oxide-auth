...
use gotham::state::State;
use hyper::header::{Authorization, ContentLength, ContentType, Location};
use hyper::{Body, Headers, Method, Request, Response, StatusCode, Uri};
pub use oxide_auth_core::code_grant::endpoint::{AuthorizationResult, OAuthError};
pub use oxide_auth_core::code_grant::endpoint::{OwnerAuthorization, OwnerAuthorizer};
pub use oxide_auth_core::code_grant::prelude::*;
use oxide_auth_core::dev::*;
...
impl WebRequest for ResolvedRequest {
    ...
    fn urlbody(&mut self) -> Result<QueryParameter, ()> {
        self.body
            .as_ref()
            .map(|body| {
                QueryParameter::SingleValue(SingleValueQuery::StringValue(Cow::Borrowed(body)))
            })
            .ok_or(())
    }
...
impl ReadyAuthorizationCodeRequest {
    ...
    where
        A: Fn(&Request, &PreGrant) -> OwnerAuthorization<Response>,
    {
        flow.handle(self.0).complete(ResolvedOwnerAuthorization {
            handler: authorizer,
        })
    }
}
